const CATEGORIES = ['bug', 'deadline', 'meeting', 'production'];
...
function generateExcuse(category) {
  ...
  const subject = pickRandom(fragments.subjects);
  const verb = pickRandom(fragments.verbs);
  const reason = pickRandom(fragments.reasons);
